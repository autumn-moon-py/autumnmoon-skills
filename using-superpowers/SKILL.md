...
技能使用 Claude Code 的工具名称。非 CC 平台：查看 `references/hermes-tools.md`（Hermes Agent）、`references/codex-tools.md`（Codex）了解工具对应关系。Gemini CLI 用户通过 GEMINI.md 自动获得工具映射。
...
**触发：** 任何编码任务——强制最懒但有效的解决方案。YAGNI 原则、标准库优先、最短路径。

**适用：** 编写、添加、重构、修复、审查或设计代码；选择库或依赖；用户说"最简方案""懒人模式""YAGNI""别过度工程"。

**不适用：** 非编码请求（通用知识、翻译、摘要）。

### full-output-enforcement

**触发：** 任何需要完整输出的任务——禁止截断、禁止占位符、禁止 `// ...` 模式。

**适用：** 用户要求完整文件、完整实现；需要确保代码可运行而非描述性占位。
...
**触发：** 多步骤任务需要文件驱动的持续规划——创建 task_plan.md、findings.md、progress.md 作为"磁盘上的工作记忆"。

**适用：** 5+ 工具调用的复杂任务；研究任务；需要跨会话恢复的规划；需要进度跟踪和错误日志。

**不适用：** 简单问题；单文件编辑；快速查找。

### writing-skills

**触发：** 创建新技能、编辑现有技能或在部署前验证技能是否有效时使用。

**适用：** 编写新的 SKILL.md；编辑现有技能；测试技能是否被智能体正确遵循；需要 TDD 流程应用于流程文档。
