---
name: writing-plans
description: Use when 需要根据规格说明或需求为多步骤开发任务编写实现计划，且尚未开始编码时
---
...
编写全面的实现计划，假设工程师对我们的代码库零上下文，且品味存疑。记录他们需要知道的一切：每个任务要修改哪些文件、代码、测试、可能需要查阅的文档、如何测试。将整个计划拆成小步骤任务。DRY。YAGNI。TDD。版本管理操作必须遵守用户授权和仓库规则。

假设他们是有经验的开发者，但对我们的工具链和问题领域几乎一无所知。假设他们不太擅长测试设计。

**开始时宣布：** "我正在使用 writing-plans 技能创建实现计划。"

**上下文：** 默认在当前工作目录中生成计划。只有用户明确要求，且仓库规则允许时，才使用或创建 Git worktree。
...
**生成实现计划不代表获得 Git 操作授权。** 用户直接指令和仓库中的 `AGENTS.md`、`CLAUDE.md`、`GEMINI.md` 等规则优先于本技能。

| 场景 | 计划要求 |
|------|----------|
| 用户未明确授权 Git 操作 | 不添加 `git add`、`git commit`、`git push`、分支或 worktree 步骤 |
| 用户明确要求提交 | 在仓库规则允许的前提下，列出精确的提交步骤 |
| 仓库规则禁止自动提交 | 不得添加可执行的提交步骤，可按用户要求提供建议提交信息 |

### 红线

未经用户明确授权，不得在计划中加入或在计划阶段执行以下操作：
...
| 借口 | 现实 |
|------|------|
| "模板默认包含 Commit" | 模板不能替代用户授权。 |
| "只是写进计划，不会立即执行" | 执行代理会把计划步骤视为可执行指令。 |
| "worktree 更安全" | worktree 仍会改变 Git 状态，必须先获得授权。 |
...
确认只涉及本任务列出的文件，并记录已执行的验证命令及结果。除非用户明确授权，否则不要添加任何 Git 操作。
````
...
- 精确的命令和预期输出
- DRY、YAGNI、TDD
- 未经用户明确授权，不得把 Git 操作作为任务步骤或完成条件
