...
digraph process {
    ...
    "使用 using-superpowers:finishing-a-development-branch" [shape=box style=filled fillcolor=lightgreen];
    ...
    "分派最终代码审查子智能体审查整体实现" -> "使用 using-superpowers:finishing-a-development-branch";
}
```
...
**必需的工作流技能：**
- **using-superpowers:using-git-worktrees** - 必需：在开始前建立隔离工作区
- **using-superpowers:writing-plans** - 创建本技能执行的计划
- **using-superpowers:requesting-code-review** - 审查子智能体的代码审查模板
- **using-superpowers:finishing-a-development-branch** - 所有任务完成后收尾

**子智能体应使用：**
- **using-superpowers:test-driven-development** - 子智能体对每个任务遵循 TDD

**替代工作流：**
- **using-superpowers:executing-plans** - 用于并行会话而非同会话执行
